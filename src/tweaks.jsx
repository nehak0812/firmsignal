...
export function TweaksPanel({ title = 'Tweaks', children }) {
  ...
  useEffect(() => {
    // Enable by default or respond to messages
    setOpen(true);
  }, []);
...
const __TwkCheck = ({ light }) => (
  <svg viewBox="0 0 14 14" aria-hidden="true" style={{ width: 13, height: 13, position: 'absolute', top: 6, left: 6 }}>
...
export function TweakColor({ label, value, options, onChange }) {
  ...
  return (
    <TweakRow label={label}>
      <div className="twk-chips" role="radiogroup" style={{ display: 'flex', gap: 6 }}>
        {options.map((o, i) => {
          ...
          return (
            <button key={i} type="button" className="twk-chip" role="radio"
                    ...
                    style={{ background: hero, position: 'relative', width: 56, height: 22, border: '.5px solid rgba(0,0,0,.1)', borderRadius: 6, padding: 0 }}
                    onClick={() => onChange(o)}>
              {sup.length > 0 && (
                <span style={{ position: 'absolute', top: 0, bottom: 0, right: 0, width: '34%', display: 'flex', flexDirection: 'column' }}>
                  {sup.map((c, j) => <i key={j} style={{ background: c, flex: 1 }} />)}
                </span>
              )}
