...
// ============== ADVISORY CHAT VIEW ==============
function AdvisoryChatView({ data, onShowToast }) {
  const [agent, setAgent] = useState('advisor');
  const [message, setMessage] = useState('');
  const [chatHistory, setChatHistory] = useState({
    advisor: [
      { role: 'assistant', content: 'Greetings. I am your Strategic Advisor. Ask me about competitive threats, sovereign AI plays, partner landscape developments, or 90-day execution blueprints.' }
    ],
    analyst: [
      { role: 'assistant', content: 'Principal Analyst ready. Ask me to compile aggregated briefings, analyze market distributions, or draft a board-ready report for printing.' }
    ],
    researcher: [
      { role: 'assistant', content: 'Lead Researcher online. I can perform live external searches on consulting and tech firms using web search tool access.' }
    ]
  });
  const [loading, setLoading] = useState(false);
  const [ragGrounding, setRagGrounding] = useState([]);

  const currentHistory = chatHistory[agent] || [];
  ...
  const handleSend = async (e) => {
    ...
    setChatHistory(prev => ({
      ...prev,
      [agent]: updatedHistory
    }));

    try {
      const apiKey = localStorage.getItem('fs_apikey') || '';
      
      // Perform local keyword match purely for frontend visualization of RAG grounding
      const terms = userMsg.toLowerCase().split(/\s+/).filter(t => t.length > 2);
      const matched = data.filter(sig => {
        const text = [sig.title, sig.takeaway, sig.summary, sig.firm, sig.signal].join(' ').toLowerCase();
        return terms.some(term => text.includes(term));
      }).slice(0, 3);
      setRagGrounding(matched);
      ...
      setChatHistory(prev => ({
        ...prev,
        [agent]: [...updatedHistory, { role: 'assistant', content: resData.reply }]
      }));
    } catch (err) {
      onShowToast(`Chat failed: ${err.message}`);
      setChatHistory(prev => ({
        ...prev,
        [agent]: [...updatedHistory, { role: 'assistant', content: `Error: ${err.message}. Please verify your server connection or API key.` }]
      }));
    } finally {
      setLoading(false);
    }
  ...
  const agentDetails = {
    advisor: {
      ...
      roleDescription: 'Provides opinionated, board-level strategic advice, assesses competitive threats, and crafts 90-day action plans.',
      accent: 'var(--accent)'
    },
    analyst: {
      name: 'Principal Analyst',
      roleDescription: 'Drafts highly structured, markdown executive summaries, compiles sector metrics, and formats board reports.',
      accent: 'var(--info)'
    },
    researcher: {
      name: 'Lead Researcher',
      roleDescription: 'Enables live web search tools to fetch real-time industry news, sweeps the web, and answers recent queries.',
      accent: 'var(--pos)'
    }
  }[agent];
  ...
  return (
    <div className="chat-view">
      <div className="aw-head" style={{ marginBottom: 18 }}>
        <div>
          <div className="aw-eyebrow">Executive Advisory</div>
          <h1 className="aw-title">FirmSignal <em>Advisory Chat</em></h1>
          <p className="aw-sub">Consult with specialized virtual executive advisors grounded in your proprietary signal intelligence database.</p>
        </div>
      </div>

      <div className="chat-layout">
        <div className="chat-sidebar">
          <div className="sb-label" style={{ marginBottom: 10 }}>Select Agent</div>
          <button className={`agent-btn ${agent === 'advisor' ? 'active' : ''}`} onClick={() => setAgent('advisor')} style={{ '--btn-accent': 'var(--accent)', width: '100%', marginBottom: 8 }}>
            <span className="agent-indicator" style={{ background: 'var(--accent)' }} />
            Strategic Advisor
          </button>
          <button className={`agent-btn ${agent === 'analyst' ? 'active' : ''}`} onClick={() => setAgent('analyst')} style={{ '--btn-accent': 'var(--info)', width: '100%', marginBottom: 8 }}>
            <span className="agent-indicator" style={{ background: 'var(--info)' }} />
            Principal Analyst
          </button>
          <button className={`agent-btn ${agent === 'researcher' ? 'active' : ''}`} onClick={() => setAgent('researcher')} style={{ '--btn-accent': 'var(--pos)', width: '100%', marginBottom: 18 }}>
            <span className="agent-indicator" style={{ background: 'var(--pos)' }} />
            Lead Researcher
          </button>

          <div className="agent-desc-card">
            <h4>{agentDetails.name}</h4>
            <p>{agentDetails.roleDescription}</p>
            {agent !== 'researcher' && (
              <div className="rag-badge">
                <span className="rag-dot" />
                Database RAG Grounded
              </div>
            )}
            {agent === 'researcher' && (
              <div className="rag-badge tools">
                <span className="rag-dot" style={{ background: 'var(--pos)' }} />
                Web Search Enabled
              </div>
            )}
          </div>
        ...
        <div className="chat-main">
          <div className="chat-history-container">
            {currentHistory.map((m, idx) => (
              <div key={idx} className={`chat-bubble-wrap ${m.role}`}>
                <div className="chat-avatar">{m.role === 'user' ? 'CEO' : agentDetails.name[0]}</div>
                <div className="chat-bubble">
                  <div className="chat-bubble-meta">{m.role === 'user' ? 'Executive Reader' : agentDetails.name}</div>
            ...
            {loading && (
              <div className="chat-bubble-wrap assistant loading">
                <div className="chat-avatar">{agentDetails.name[0]}</div>
                <div className="chat-bubble">
                  <div className="chat-bubble-meta">{agentDetails.name} is thinking...</div>
          ...
          {ragGrounding.length > 0 && (
            <div className="rag-visualiser">
              <span style={{ color: 'var(--accent)', fontWeight: 500 }}>RAG Grounding:</span> Found {ragGrounding.length} matching signals in DB (e.g. "{ragGrounding[0].title.slice(0, 45)}...")
            </div>
          )}
          ...
          <form onSubmit={handleSend} className="chat-input-form">
            <input
              ...
              placeholder={`Ask the ${agentDetails.name}...`}
              disabled={loading}
            />
            <button type="submit" className="chat-send-btn" style={{ background: agentDetails.accent, color: 'var(--bg)' }} disabled={loading || !message.trim()}>
              Consult →
            </button>
          </form>
        </div>
...
Object.assign(window, {
  ...
  AiWatchView, AiSignalBlock, AdvisoryChatView,
});
